...
impl NearestAttackableTargetGoal {
        ...
        Fut: Future<Output = bool> + Send + 'static,
    {
        ...
        Self {
            track_target_goal,
            target: None,
            random_interval: to_goal_ticks(random_interval),
            target_type,
            target_predicate,
        }
    }
    ...
    pub fn with_default(
        ...
        Box::new(Self {
            track_target_goal,
            target: None,
            random_interval: to_goal_ticks(10), // DEFAULT_RANDOM_INTERVAL = 10
            target_type,
            target_predicate,
        })
    }
    ...
    fn find_target(&mut self, mob: &MobEntity) {
    ...
    fn can_start<'a>(&'a mut self, mob: &'a dyn Mob) -> GoalFuture<'a, bool> {
        Box::pin(async {
            if self.random_interval > 0
                && mob.get_random().random_range(0..self.random_interval) != 0
            {
                return false;
            }
            self.find_target(mob.get_mob_entity());
            self.target.is_some()
        })
    }
